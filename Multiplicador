...
float multiplicacion(float multiplicando, float multiplicador)
{
    ...
    else
    {
        ...
        if (nuevo_exponente > std::numeric_limits<float>::max_exponent) 
        {
            ...
            return std::numeric_limits<float>::quiet_NaN(); // Devuelve un NaN en caso de overflow
        }
        else if (nuevo_exponente < std::numeric_limits<float>::min_exponent) 
        {
            ...
            return std::numeric_limits<float>::quiet_NaN(); // Devuelve un NaN en caso de underflow
        }
        else 
        {
            ...
            while (nueva_mantisa >= 2.0f) {
                nueva_mantisa /= 2.0f;
                nuevo_exponente++; // Incrementar el exponente en la normalización
            }
